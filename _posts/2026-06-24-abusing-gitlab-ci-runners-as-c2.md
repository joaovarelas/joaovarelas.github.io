...
During installation, the GitLab Runner registers itself as a custom Windows Event Log provider under the **Application** log channel. This means runner activity - service start, job execution, errors - is written directly to the Windows Event Log under the source name `gitlab-runner`.

![image](/assets/img/posts/2026/06/e9a3681fd4c361ae0b6b5795de7e9945ef2bade7f1b5d5afea861dcf3d5e8280.png)

From a detection standpoint, this is a double-edged artifact: the presence of a `gitlab-runner` event source on a machine that has no business running CI pipelines is itself a strong indicator of compromise, and the events logged under it expose job execution activity without requiring PowerShell Script Block Logging to be enabled.
